...
OpenClaw IS the application. Business logic goes in Skills (SKILL.md), NOT in Python services. The data-api is persistence only — it stores, encrypts, and retrieves data.
</Callout>
...
<Callout type="warning" title="Zero public ports">
Aegis exposes zero ports to the public internet. All external access flows through Cloudflare Tunnel with token-based authentication.
</Callout>
