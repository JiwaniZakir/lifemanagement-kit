---
title: Contributing
description: How to contribute to Aegis
---
...
See the [Local Development](/docs/development/local-dev) guide for the full development workflow, including:
...
All code conventions are documented in `CLAUDE.md` at the project root. Key rules:
...
See the [Adding Integrations](/docs/development/adding-integrations) guide for a step-by-step walkthrough. In summary:
...
Skills are Markdown files in `skills/<name>/SKILL.md` with YAML frontmatter. They contain instructions for agents, not code. See the [Adding Skills](/docs/development/adding-skills) guide for the format.

## Adding New Hooks

Hooks are TypeScript files in `hooks/<name>/handler.ts` with a `HOOK.md` config file. See the [Adding Hooks](/docs/development/adding-hooks) guide for the format and registration.

## Security

If you discover a security vulnerability, **do not open a public issue.** Email the maintainer directly. See the [Threat Model](/docs/security/threat-model) page for the full security policy and reporting instructions.
