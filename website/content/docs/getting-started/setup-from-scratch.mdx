...
description: Complete onboarding guide for going from nothing to a running Aegis instance, including Docker installation, OpenClaw setup, WhatsApp pairing, and integration configuration.
---
...
<Callout type="tip" title="Time estimate">
The full setup takes 15-30 minutes. If you already have Docker installed and an Anthropic API key, you can be running in under 5 minutes.
</Callout>
...
The `.env` file is in `.gitignore`. Never hardcode API keys in source code or commit them to version control.
</Callout>
...
<Callout type="warning" title="Unofficial API">
The Garmin integration uses the unofficial `garminconnect` library. It may break if Garmin changes their internal API without notice.
</Callout>
...
<Callout type="info" title="API limitation">
LinkedIn's API is very limited. Posting requires an approved app through their developer program.
</Callout>
...
<Callout type="info" title="Paid API">
X API v2 requires a paid plan ($100/month for Basic tier) for read + write access.
</Callout>
