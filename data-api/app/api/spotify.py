"""Spotify API — listening history ingestion, queries, and stats."""
...
async def upsert_top_snapshot(
    ...
    return {"ok": True, "snapshot_type": body.snapshot_type, "time_range": body.time_range}


@router.get("/top/latest")
async def get_latest_top_snapshot(
    user_id: str = Query(..., max_length=36),
    snapshot_type: str = Query(..., max_length=16),
    time_range: str = Query("short_term", max_length=16),
    db: AsyncSession = Depends(get_db),
) -> dict | None:
    result = await db.execute(
...
async def sync_spotify(
    ...
    try:
        ...
        logger.error("spotify_client_import_error")
        return {"ok": False, "error": "SpotifyClient not available"}
