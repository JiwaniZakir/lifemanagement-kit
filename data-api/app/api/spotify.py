"""Spotify API — listening history, top items, and stats."""
...
class ListeningHistoryCreate(BaseModel):
    ...
    spotify_track_id: str = Field(max_length=64)
    spotify_artist_id: str = Field(max_length=64)
    spotify_album_id: str = Field(max_length=64)
    spotify_context_uri: str | None = None
    track_name: str = Field(max_length=512)
    artist_name: str = Field(max_length=512)
    album_name: str = Field(max_length=512)
    played_at_utc: str = Field(max_length=32)
    played_at_ts: int
    duration_ms: int = 0
    explicit: bool = False
    popularity: int = 0
...
async def get_top_snapshots(
    user_id: str = Query(default="default"),
    snapshot_type: str = Query(default="tracks", pattern="^(tracks|artists)$"),
    time_range: str = Query(
        default="medium_term", pattern="^(short_term|medium_term|long_term)$"
    ),
    db: AsyncSession = Depends(get_db),
) -> dict:
    result = await db.execute(
    ...
    if snapshot is None:
        return {"snapshot_type": snapshot_type, "time_range": time_range, "items": [], "count": 0}
    return {
...
@router.post("/top", status_code=201)
async def create_top_snapshot(
    ...
    await audit_log(
        db,
        action="spotify_top_snapshot_created",
        resource_type="spotify",
        metadata={"type": body.snapshot_type, "range": body.time_range},
    )
    return {"ok": True, "id": str(snapshot.id)}
