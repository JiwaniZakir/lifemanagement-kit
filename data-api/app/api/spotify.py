"""Spotify API — listening history queries, bulk upsert, and sync."""
...
class BulkUpsertRequest(BaseModel):
    ...
    plays: list[PlayCreate] = Field(..., min_length=1, max_length=50)


class TopSnapshotCreate(BaseModel):
    user_id: str = Field(max_length=36)
    snapshot_type: str = Field(max_length=16)
    time_range: str = Field(max_length=16)
    items_json: str
    item_count: int
...
def _play_to_response(row: ListeningHistory) -> PlayResponse:
    return PlayResponse(
        id=str(row.id),
        user_id=row.user_id,
...
def _snapshot_to_response(row: SpotifyTopSnapshot) -> TopSnapshotResponse:
...
@router.get("/plays/recent", response_model=list[PlayResponse])
async def get_recent_plays(
    ...
    return [_play_to_response(r) for r in result.scalars().all()]


@router.get("/plays/range", response_model=list[PlayResponse])
...
async def upsert_top_snapshot(
    ...
    existing = await db.execute(
        select(SpotifyTopSnapshot).where(
            SpotifyTopSnapshot.user_id == body.user_id,
            SpotifyTopSnapshot.snapshot_type == body.snapshot_type,
            SpotifyTopSnapshot.time_range == body.time_range,
        )
    )
    ...
    if row:
        ...
        row.item_count = body.item_count
    else:
        db.add(
            SpotifyTopSnapshot(
                user_id=body.user_id,
...
@router.get("/top/snapshot", response_model=list[TopSnapshotResponse])
async def get_top_snapshots(
    ...
    stmt = select(SpotifyTopSnapshot).where(SpotifyTopSnapshot.user_id == user_id)

    if snapshot_type:
        stmt = stmt.where(SpotifyTopSnapshot.snapshot_type == snapshot_type)
    if time_range:
        stmt = stmt.where(SpotifyTopSnapshot.time_range == time_range)

    stmt = stmt.order_by(desc(SpotifyTopSnapshot.created_at))

    result = await db.execute(stmt)
    return [_snapshot_to_response(r) for r in result.scalars().all()]
...
async def sync_spotify(
    ...
    try:
        ...
        logger.error("spotify_sync_http_error", error=type(exc).__name__)
