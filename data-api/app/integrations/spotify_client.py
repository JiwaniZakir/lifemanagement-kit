"""Spotify Web API integration — listening history and top artists/tracks."""
...
class SpotifyClient(BaseIntegration):
    ...
    async def _refresh_access_token(self) -> str:
        ...
        await self._audit(action="spotify_token_refresh", resource_type="spotify")
        return access_token
    ...
    async def _api_get(self, path: str, params: dict | None = None) -> dict:
        ...
        if resp.status_code != 200:
            raise SpotifyClientError(f"Spotify API {path} returned {resp.status_code}")
        return resp.json()
    ...
    async def _sync_recently_played(self) -> int:
        ...
        for item in items:
            ...
            stmt = (
                ...
                .values(
                    ...
                    spotify_context_uri=context.get("uri"),
    ...
    async def sync(self) -> None:
        inserted = await self._sync_recently_played()
        snapshots = await self._sync_top_snapshots()
        await self.db.flush()
        await self._audit(
            action="spotify_sync",
            resource_type="spotify",
            metadata={"inserted_plays": inserted, "snapshots_updated": snapshots},
        )
        self._log.info("spotify_sync_complete", inserted=inserted, snapshots=snapshots)
